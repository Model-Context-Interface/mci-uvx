...
def test_apply_multiple_filters():
    ...
    try:
        ...
        # Filter for api tools, then exclude deprecated
        tools = ToolManager.apply_multiple_filters(
            wrapper, ["tags:api", "without-tags:deprecated"]
        )
        assert len(tools) == 2
...
def test_apply_multiple_filters_empty_list():
    """Test that empty filter list returns all tools."""
    schema = {
        "schemaVersion": "1.0",
        "tools": [
            {
    ...
    try:
        wrapper = MCIClientWrapper(schema_path)
        tools = ToolManager.apply_multiple_filters(wrapper, [])
        assert len(tools) == 2
        assert {t.name for t in tools} == {"tool1", "tool2"}
    finally:
        Path(schema_path).unlink()
