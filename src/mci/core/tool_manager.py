...
class ToolManager:
    ...
    @staticmethod
    def apply_multiple_filters(client: MCIClientWrapper, filter_specs: list[str]) -> list[Tool]:
        """
        Apply multiple filter specifications sequentially.

        Filters are applied in order, with each filter operating on the results
        of the previous filter. This allows combining multiple filter types.

        Args:
            client: MCIClientWrapper instance to apply filters on
            filter_specs: List of filter specification strings

        Returns:
            Filtered list of Tool objects after applying all filters

        Raises:
            ValueError: If any filter specification is invalid

        Example:
            >>> wrapper = MCIClientWrapper("mci.json")
            >>> tools = ToolManager.apply_multiple_filters(
            ...     wrapper,
            ...     ["tags:api", "except:deprecated_tool"]
            ... )
        """
        if not filter_specs:
        ...
        # we might need to create a temporary wrapper for each intermediate result
        for filter_spec in filter_specs:
            filter_type, values = ToolManager.parse_filter_spec(filter_spec)

            # Filter current_tools based on the filter type
            if filter_type == "only":
                current_tools = [t for t in current_tools if t.name in values]
            elif filter_type == "except":
                current_tools = [t for t in current_tools if t.name not in values]
            elif filter_type == "tags":
                current_tools = [
                    t for t in current_tools if t.tags and any(tag in t.tags for tag in values)
                ]
            elif filter_type == "without-tags":
                current_tools = [
                    t
                    for t in current_tools
                    if not t.tags or not any(tag in t.tags for tag in values)
                ]
            elif filter_type == "toolsets":
                current_tools = [
                    t
                    for t in current_tools
                    if hasattr(t, "toolset_source") and t.toolset_source in values
                ]
